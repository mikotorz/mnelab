...
- Add an optional Decimate setting to Run ICA (and the Pipeline's Run ICA step) that fits on only every Nth sample, cutting calculation time at the cost of using less data; unchecked by default ([fc5f979](https://github.com/mikotorz/mnelab/commit/fc5f9797b9af2994715e927a55cffda38eac2e20) by [mikotorz](https://github.com/mikotorz))
...
- Add a Preset row (save/select/delete named presets, plus Restore Defaults) to the Filter Data, Create Epochs, Detect Artifacts, and Auto-Select ICs dialogs ([#21](https://github.com/mikotorz/mnelab/pull/21) by [mikotorz](https://github.com/mikotorz))

### 🔧 Fixed
- Fix autosave silently overwriting a previously-saved project file after closing all data sets and starting a new, unrelated session, caused by `Model.project_path` not being reset when the session's data sets were cleared ([#20](https://github.com/mikotorz/mnelab/pull/20) by [mikotorz](https://github.com/mikotorz))
- Fix Run ICA's "Number of Components" defaulting to the total channel count instead of the count of good (non-bad) data channels actually used to fit, which raised `ica.n_components (N) cannot be greater than len(picks) (M)` whenever any channels were marked bad ([21f9c6b](https://github.com/mikotorz/mnelab/commit/21f9c6b860fd5bf0f5191e87fcb565367a53d5ef) by [mikotorz](https://github.com/mikotorz))
- Fix Run ICA and Plot ERDS maps hanging indefinitely on the "Calculating..." dialog with no CPU/RAM activity when the background calculation raised an exception, instead of surfacing the error ([fc5f979](https://github.com/mikotorz/mnelab/commit/fc5f9797b9af2994715e927a55cffda38eac2e20) by [mikotorz](https://github.com/mikotorz))
- Fix "Plot Evoked" with topomaps squeezing its layout (and the colorbar drifting to an unstable width) when drag-selecting a time range interactively, caused by an mne-python 1.13.0 regression — resolved by pinning `mne < 1.13` after two interim fixes proved insufficient ([#13](https://github.com/mikotorz/mnelab/issues/13) by [mikotorz](https://github.com/mikotorz))
